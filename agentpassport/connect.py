"""Native cognis-connect emit for agentpassport — forward findings to any platform.

Maps agentpassport's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    agentpassport ... --format json | agentpassport-emit --to stix
    agentpassport-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['description'] = rec.get('details', '')
        out['tags'] = rec.get('tags', []).copy()
